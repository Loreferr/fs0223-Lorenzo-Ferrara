...
console.log(deleteOne("stringa", 12 > 11))
/* ESERCIZIO 5
  Crea una funzione chiamata "onlyLetters" che riceve una stringa come parametro e la ritorna eliminando tutte le cifre numeriche.

  Es.: onlyLetters("I have 4 dogs") => ritorna "I have dogs"
*/
function onlyLetters(c) {
...
function whatDayIsIt() {
  ...
  return date.toLocaleDateString('it-IT', testo)
...
function howManyDays(dateString) {
  ...
  let diffInDays = Math.floor((today - secondDate) / (1000 * 60 * 60 * 24));
  return diffInDays;
}
//funziona con le date americane
console.log(howManyDays('4/10/2023'))
...
console.log(isTodayMyBirthday('9/16/1995'));
// Arrays & Oggetti
...
function sumAllTheYears() {
  sum = 0
  for (const numAnni of movies) {
    sum += Number(numAnni.Year)
  }
return sum
...
function searchAndDivide(str) {
  ...
  for (let i = 0; i < movies.length; i++) {
    if (movies[i].Title.toLowerCase().includes(str.toLowerCase())) {
      recipe.match.push(movies[i])
      
    } else{

      recipe.unmatch.push(movies[i])
    }
    return recipe
  }
