...
for(let index = 0; index < starWarsCharacters.length; index++){
  const name = starWarsCharacters[index].name;
...
let i = 0
  while(i < starWarsCharacters.length ){
    massaTotale += parseInt (starWarsCharacters[i].mass)
    i++
  }
...
for(i = 0 ; i < starWarsCharacters.lenght; i++){
  const gender = starWarsCharacters[i].gender;
  if(starWarsCharacters[i].gender == "n/a"){
    starWarsCharacters[1].gender = "robot"
